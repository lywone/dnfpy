...
def _decompose_equip():
    ...
    for i in range(a.DECOMPOSE_MAX_TRY):  # 最多 10 轮
        ...
        if frame is None:       # 截图失败
            time.sleep(1)       # 等 1s
            continue            # 继续循环
        # 检测金黄「分解」按钮（GO）——它出现说明分解主弹框已打开（比顶部横栏模板更稳）
        gs, gc = a.detect_button(frame, a.TEMPLATE_DECOMPOSE_GO, roi=a.decompose_go_roi(frame))
        if gs is not None and gs >= a.DECOMPOSE_TH and gc:  # GO 按钮匹配达标 → 弹框已打开
            print(f"[分解] 分解弹框已打开（金黄「分解」按钮 {gs:.3f} @{gc}）")  # 打印日志
        ...
        if ds is not None and ds >= a.DECOMPOSE_TH and dc:  # 匹配达标
            ...
            a.adb_tap(dc[0], dc[1])  # 点击分解入口按钮（打开分解弹框）
    ...
    for i in range(a.DECOMPOSE_MAX_TRY):  # 最多 10 轮点金黄分解
        ...
        time.sleep(1)           # 等 1s 再试
    # 等待 2s → 检查第一个「提示」框的确认按钮（TEMPLATE_DECOMPOSE_CONFIRM），5 次 × 2s
    # 不依赖 hint 标题模板（2560x1440 下匹配分仅 0.33），直接检测确认按钮：按钮出现=提示框已弹
    time.sleep(2.0)             # 点完分解先等 2s 让弹窗弹出
    outer_ok = False            # 第一个提示框确认是否点到
    for i in range(5):          # 最多检查 5 次
        frame = a.capture_hdmi()  # 截取当前画面
        if frame is not None:   # 画面有效
            cs, cc = a.detect_button(frame, a.TEMPLATE_DECOMPOSE_CONFIRM, roi=a.decompose_popup_roi(frame))  # 中央检测确认按钮
            print(f"[分解] 第 {i+1}/5 次 confirm 分数={cs if cs is not None else 'None'}（阈值 {a.DECOMPOSE_TH}）@{cc}")  # 调试
            if cs is not None and cs >= a.DECOMPOSE_TH and cc:  # 确认按钮出现
                print(f"[分解] 第一个提示框确认按钮出现（{cs:.3f} @{cc}），点击…")  # 打印日志
                a.adb_tap(cc[0], cc[1])  # 点实时检测到的确认按钮
                outer_ok = True  # 标记已点
                break           # 跳出检查循环
        time.sleep(2.0)         # 等 2s 再查
    if not outer_ok:            # 5 次都没等到
        print("[分解] 5 次检查均未出现第一个提示框确认按钮，关闭分解流程，继续返回主页面…")  # 文字提示
        ...
        for i in range(5):      # 最多检查 5 次
            ...
            if frame is not None:  # 画面有效
                ...
                if vs is not None and vs >= a.DECOMPOSE_TH and vc:  # 高价值二次确认出现
                    print(f"[分解] 高价值二次确认出现（{vs:.3f}），等 2s 让对话框稳定后再检测确认按钮…")  # 打印日志
                    time.sleep(2.0)  # 等 2s 让对话框完全弹出稳定（避免动画中误匹配到错误位置）
                    frame2 = a.capture_hdmi()  # 重新截图
                    if frame2 is None:  # 截图失败
                        a.adb_tap(a.DECOMPOSE_INNER_CONFIRM[0], a.DECOMPOSE_INNER_CONFIRM[1])  # 兜底点固定坐标
                    else:
                        cs, cc = a.detect_button(frame2, a.TEMPLATE_DECOMPOSE_CONFIRM, roi=a.decompose_popup_roi(frame2))  # 重新检测确认按钮
                        print(f"[分解] 高价值框稳定后 confirm 分数={cs if cs is not None else 'None'}@{cc}")  # 调试
